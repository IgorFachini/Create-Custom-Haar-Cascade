...
def get_neg_images(url, neg_image_folder):
    images_urls = urllib.request.urlopen(url).read().decode('utf8')
    pic_num = 1
    for iurl in images_urls.split('\n'):
        try:
            print(iurl)
            # urlgrab(iurl, neg_image_folder + '/' + str(pic_num) + '.jpg'
            #         , timeout=20)
            img = cv2.imread(neg_image_folder + '/' + str(pic_num)
                             + '.jpg', cv2.IMREAD_GRAYSCALE)
            resize_img = cv2.resize(img, (100, 100))
            cv2.imwrite(neg_image_folder + '/' + str(pic_num) + '.jpg',
                        resize_img)
            pic_num += 1
...
def create_samples(inp_dir, num):
    for file in os.listdir(inp_dir):
        cmd = 'opencv_createsamples -img ' + inp_dir + '/' + file \
            + ' -bg bg.txt -info positives/info' + file.split('.')[0] \
            + '.lst -pngoutput positives -maxxangle 0.5 -maxyangle 0.5 -maxzangle 0.5 -num ' \
            + num + ' -bgcolor 000'
        os.system(cmd)
...
Choice: '''))
    ...
    if choice == 1:
        ...
        print('-------------------------------------------\n')
        url = \
            str(input('Enter the URL that contains the download links (should look like http://image-net.org/api/text/imagenet.synset.geturls?wnid=n03642806): '
                ))
...
''')
        ...
        input('Put the one of each ugly images in the '
                  + ugly_folder + ' folder and press ENTER....')
        ...
        resize_x = \
            int(input('Enter the resize dimension in x-axis (recommended 50): '
                ))
        resize_y = \
            int(input('Enter the resize dimension in y-axis (recommended 50): '
                ))
        resize_input_img(input_img_dir, resize_x, resize_y,
                         output_img_dir)
        input('Press ENTER to continue...')
    elif choice == 4:
        ...
        pics = len(os.listdir(input_img_dir)) \
            * len(os.listdir(neg_img_dir)) * 100
...
Merging every info files into 1 info.lst''')
        ...
        cmd = 'opencv_createsamples -info positives/info.lst -num ' \
            + str(num - 1) + ' -w 25 -h 25 -vec positives.vec'
        os.system(cmd)
        print('''
...
''')
        ...
        print('Number of positive samples available ' \
            + str(len(os.listdir('positives'))))
        print('Number of negative samples available ' \
            + str(len(os.listdir('negatives'))) + '\n')

        pos = \
            input('Enter number of positive samples you want to use for training: '
                      )
        neg = \
            input('Enter number of negative samples you want to use for training: '
                      )
        num = input('Enter number of stages: ')
        if not os.path.isdir('output_cascade'):
            os.makedirs('output_cascade')
        cmd = \
            'opencv_traincascade -data output_cascade -vec positives.vec -bg bg.txt -numPos ' \
            + pos + ' -numNeg ' + neg + ' -numStages ' + num \
            + ' -w 25 -h 25 -precalcBufSize 1024 -precalcIdxBufSize 1024'
        os.system(cmd)
        input('Press ENTER to continue...')
    elif choice == 6:
